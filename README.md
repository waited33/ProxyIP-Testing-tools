...
>   自动下载**默认直连优先**：直连成功就不走代理；仅当直连全部失败（被墙）时，才依次尝试
>   Windows 系统代理（浏览器同款）→ PAC 解析出的代理 → 环境变量代理 → 常见本地代理端口
>   （Clash 7890、v2rayN 10808/10809 等）作回退。若最终走了代理，命令行会打印
>   `(经代理 http://...)` 字样；mmdb 是同一份数据文件，与下载方式无关。
...
- **ASN 列空白**：`GeoLite2-ASN.mmdb` 缺失且自动下载失败所致。可先手动获取该文件放到程序目录，
  或用 `-asndb` 指向已有数据库；若浏览器能下但程序下载失败，多半是**直连被墙**，程序会自动改走
  **Windows 系统代理（浏览器同款）/PAC/本地代理端口**兜底（直连优先、代理仅作回退），无需手动干预。
- **检测/测速全程强制直连，与是否挂代理无关**：TCP 建连走原生 `net.Dial`，握手/测速的 `http.Transport`
  显式禁用了代理（`Proxy: nil`）。即使开着 Clash/系统代理、环境变量残留 `HTTP_PROXY`，探测请求也
  **不会**被劫持到代理，测出的出口IP/地区/延迟/速度全部代表 "无代理直连" 的真实可达性——
  这正是 "没挂代理验证出来才准确" 的含义。需要验证挂了代理的效果时，请用支持指定代理的其它工具。
